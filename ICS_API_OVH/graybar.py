...
def graybar(pdp_url, sku, vendor):
    try:
        url_request = str()
        error = dict()
        try:
        ...
        if response.xpath('//div[@class="flex-row first-column pdpDesktopView"]//div[@class="sell-uom"]/span[last()]'):
            ...
            min_qty =re.sub('\D','',response.xpath('//div[@class="flex-row first-column pdpDesktopView"]//div[@class="sell-uom"]/span[last()]/text()').get().replace('$', '').replace(',', '').strip())
        ...
        if 'Volume discounts available' in url_request.text:
            for tr in response.xpath('//table[@class="volume__prices"]//tr[./td]'):
                ...
                original_price['min_qty'] = re.findall("\d+", all_td[0])
...
#     data = graybar(
#         pdp_url="https://www.graybar.com/perforated-panel-steel-medium-type-1-panel-enclosure-32-x-26-5-in-/p/22000362",
#         sku="22000362",
#         vendor='graybar')
#     # print json :-
#     print(data)
